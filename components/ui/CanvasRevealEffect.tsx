...
const DotMatrix: React.FC<DotMatrixProps> = ({
  colors = [[0, 0, 0]],
  opacities = [0.04, 0.04, 0.04, 0.08, 0.08, 0.08, 0.08, 0.14],
  totalSize = 4,
  dotSize = 2,
  shader = "",
  center = ["x", "y"],
  ...
  const uniforms: Uniforms = useMemo(() => ({
    u_colors: {
      value: colors.map((color) => new THREE.Vector3(color[0] / 255, color[1] / 255, color[2] / 255)), // ✅ Fixed Type
    },
    ...
    <Shader
      source={`precision mediump float;
        ...
        void main() {
          ...
          ${center.includes("x") ? "st.x -= mod(u_resolution.x, u_total_size) * 0.5;" : ""}
          ${center.includes("y") ? "st.y -= mod(u_resolution.y, u_total_size) * 0.5;" : ""}
...
interface Uniforms {
  [key: string]: { value: number | number[] | THREE.Vector2 | THREE.Vector3 | THREE.Vector3[] };
}
...
const ShaderMaterial: React.FC<{ source: string; uniforms: Uniforms }> = ({ source, uniforms }) => {
  const { size } = useThree();
